...
const ReviewFile = ({ onChange }: ReviewFileProps) => {
	...
	async function handleAddFile(e: React.ChangeEvent<HTMLInputElement>) {
		...
		const avifFiles = await Promise.all(fileList.map((f) => encodeToAvif(f)));
		const mergedFiles = _.unionBy(files, avifFiles, "name");
	...
	return (
		<div className="grid grid-cols-6 gap-2">
			{files?.map((f) => (
				<div
					key={f.name}
					className="aspect-square bg-neutral-50 rounded overflow-hidden relative"
				>
					{f && (
